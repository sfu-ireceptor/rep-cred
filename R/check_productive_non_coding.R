...
fullCheckNonProductiveSeqs <- function(file){
  seqs <- findNonProductiveSeqs((file)) #Returns vector containing sequences and their row num if they are non productive
  print(seqs)
  total_num=length(seqs)
  converted_seqs <- checkForReverseComplement(file,seqs)
...
getFullRatioNonProductive <- function(file){
  ...
  non_productive=0
  na =0 
  for(prod_row in file$productive){
  switch (as.character(prod_row),
  ...
  return(list(prod = productive, non_prod=non_productive,not_specified = na))
}
