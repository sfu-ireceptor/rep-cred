...
detail_nucleotide <- function(non, repertoire, non_nucs, total_pos) {
  ...
  cat(paste0(non, ": found in ", n_rows, " sequence(s). ", n_total, " in total (", perc, "%)\n"))
...
check_nucleotides <- function(repertoire) {
  ...
  if(length(nons) > 0) {
    ...
    x = lapply(nons, detail_nucleotide, repertoire=repertoire, non_nucs=non_nucs, total_pos=total_pos)
  } else {
    cat("None found.")
  }
}
...
plot_base_composition = function(recs, title, pos=1, end_pos=999, r_justify=F) {
  ...
  x = do.call('rbind', lapply(seq(min_pos,max_pos), nucs_at, seqs=recs, filter=F))
  
  g = ggplot(data=x, aes(x=pos, fill=nuc)) +
    scale_fill_brewer(palette='Dark2') +
    geom_bar(stat="count") +
    labs(x='Position', y='Count', fill='', title=title) +
