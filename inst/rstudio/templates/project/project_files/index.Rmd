...
>>>>>>> 6e53668 (fix (maybe?) for non existing in_ref column)
```
...
ggplot(freq_table,aes(x=call,y=Freq,fill=sequence))+geom_bar(stat="identity")+guides(fill = "none", x=guide_axis(angle=90))+labs(x="Gene",y="Number of sequences present")
```


`r  if(chimera){"Below contain CDR3 sequences with more than one v_call:"}`

```{r warning = TRUE, eval=chimera} 
options(width = 2000)
num_to_display=0
