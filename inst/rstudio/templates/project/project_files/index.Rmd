...
params:
   ...
   echo:  FALSE
   cache: FALSE
   rep:  !r system.file("extdata", "P4_I4.tsv", package = "repcred")
   outdir: !r tempdir()
   genome_file: !r NA
...
```{r, fig.cap= "A breakdown of the type of non-productive sequences. The x-axis is the non-productive type, and the y-axis is the abdunce count. The red color represents sequences that has both a stop codon and the V-J is not in frame, gray color represents sequences that are in either types.", warning=TRUE, eval=non_prod}
...
length_info %>% dplyr::group_by(!!as.name("region")) %>%
  dplyr::summarise(
...
hot_data <- getHotspotCountDistribution(repertoire)
cold_data <- getColdspotCountDistribution(repertoire)
gc_data <- getGCContentDistribution(repertoire)
